export const agentsTemplate_zh = `# OpenSpec 指南
...
4. **按顺序实施任务** - 按顺序完成
5. **确认完成** - 确保 \`tasks.md\` 中的每一项在更新状态前都已完成
...
### 需求措辞
- 使用 SHALL/MUST 表示规范性需求（除非有意使用非规范性，否则避免使用 should/may）
...
- ADDED：引入可以独立作为需求的新功能或子功能。当变更是正交的（例如，添加"斜杠命令配置"）而不是改变现有需求的语义时，首选 ADDED。
- MODIFIED：更改现有需求的行为、范围或验收标准。始终粘贴完整的、更新的需求内容（标题 + 所有场景）。归档程序将用您在此处提供的内容替换整个需求；部分增量将丢失先前的详细信息。
